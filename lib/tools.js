...
async function localFallbackExtract(message) {
  ...
  // Also try to search MongoDB for any matching drug names
  try {
    // Extract potential drug name (last capitalized word or word after preposition)
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (word.length > 3 && /^[A-Z]/.test(word)) {
        ...
        // Search MongoDB for this potential drug name
