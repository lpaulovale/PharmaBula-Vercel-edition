    ...
    const res = await fetch(HF_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Você é um extrator de nomes de medicamentos. Dada uma mensagem do usuário, extraia APENAS os nomes de medicamentos ou princípios ativos mencionados. Responda SOMENTE com um array JSON de strings, sem explicação. Se nenhum medicamento for mencionado, responda com []. Exemplos:
...
async function executeTools(message, mode, apiKey) {
  ...
  if (drugNames.length === 0) {
    ...
    // Single drug — get bula data + find generic versions
    for (const name of drugNames) {
      const bulaResult = toolGetBulaData(name, mode);
      toolResults.push(bulaResult);
      if (bulaResult.found) {
      ...
      // Search ANVISA for all registered versions (generics, similar, reference)
