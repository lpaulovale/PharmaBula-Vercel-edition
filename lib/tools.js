 ...
 * Used when the LLM extraction call fails (timeout, rate limit, etc.)
...
function localFallbackExtract(message) {
  ...
  // Check against sample data names
  ...
  // Also check some common aliases
  const aliases = {
...
async function executeTools(message, mode, apiKey) {
  let drugNames = await extractDrugNames(message, apiKey);
  console.log("Drugs detected by LLM:", drugNames);

  // Fallback: if LLM extraction failed, try local keyword matching
  if (drugNames.length === 0) {
    drugNames = localFallbackExtract(message);
    console.log("Drugs detected by local fallback:", drugNames);
  }
  ...
  if (drugNames.length === 0) {
    // Still no drug found — try word-by-word search against sample data
    const words = message.toLowerCase().split(/[\s,.;:!?]+/).filter(w => w.length > 3);
    for (const word of words) {
      const bulaResult = toolGetBulaData(word, mode);
      if (bulaResult.found) {
        drugNames.push(word);
