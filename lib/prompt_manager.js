...
when NOT to take it
- Always include: "Consulte sempre um médico ou farmacêutico antes \
de tomar qualquer medicamento."
- Prefer get_section over get_medication_details to reduce context load
...
1. You don't know the medication registration number yet?
   → call search_medication first

2. User asks about ONE specific aspect (dosage, contraindications, \
adverse effects, interactions)?
   → call get_section with the relevant section only
   → do NOT call get_medication_details

3. User asks for complete overview OR comparative analysis of 2+ medications?
   → call get_medication_details

4. User asks about interactions between multiple drugs?
   → call check_interactions with the drug list

5. You already have the data you need?
   → stop calling tools and generate the response
...
## PONTOS QUE DEVEM SER COBERTOS NA RESPOSTA
Para garantir uma resposta completa, certifique-se de abordar os seguintes pontos:
${implicitQuestions.map((q, i) => `${i + 1}. ${q}`).join("\n")}

Nota: Estas são as perguntas implícitas que usuários tipicamente têm quando perguntam sobre ${topics.join(", ")}. Mesmo que não sejam explicitamente mencionadas na pergunta, inclua essas informações na resposta quando relevante.`;
  }
