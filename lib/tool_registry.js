...
const TOOLS = [
...

  {
    ...
    handler: async ({ drug_name, mode = "patient" }) => {
      ...
      // Try ANVISA API first to get PDF URL
      const anvisa = await readResource("anvisa://bula", { query: drug_name, bulaType });
      
      if (anvisa.found && anvisa.data) {
        console.log('[DEBUG] tool_registry get_bula_data pdfUrl:', anvisa.data.pdfUrl);
        return { 
          tool: "get_bula_data", 
          drug_name, 
          mode, 
          found: true, 
          data: {
            name: anvisa.data.name,
            company: anvisa.data.company,
            pdfUrl: anvisa.data.pdfUrl,
            textContent: anvisa.data.textContent
          }
        };
      }

      // No fallback: surface ANVISA failure explicitly
      return { 
        tool: "get_bula_data", 
        drug_name, 
        mode, 
        found: false, 
        message: `Não consegui obter a bula de '${drug_name}' na ANVISA (sem PDF disponível ou erro na API).` 
      };
    },
  },

  {
    ...
    handler: async ({ drug_name, section, mode = "patient" }) => {
      ...
      if (anvisa.found && anvisa.data && anvisa.data.textContent) {
        console.log('[DEBUG] tool_registry get_section pdfUrl:', anvisa.data.pdfUrl);
        const text = anvisa.data.textContent;
        ...
        return {
          ...
          found: !!sectionText,
          pdfUrl: anvisa.data.pdfUrl,
          data: sectionText ? { name: anvisa.data.name, section, content: sectionText, pdfUrl: anvisa.data.pdfUrl } : { pdfUrl: anvisa.data.pdfUrl },
          message: sectionText ? null : `Seção '${section}' não encontrada na bula de '${drug_name}'.`,
        };
      }
      
      // FAIL CLEANLY if no bula data from ANVISA
      return {
        tool: "get_section",
        drug_name,
        section,
        ...
        pdfUrl: null,
        data: null,
        error: `Não foi possível obter a bula de '${drug_name}' da ANVISA. Verifique a conexão ou tente novamente mais tarde.`
      };
    },
  },
