...
const TOOLS = [
...

  {
    ...
    handler: async ({ drug_name, mode = "patient" }) => {
      ...
      // Fallback to local data if ANVISA fails
...

  {
    ...
    handler: async ({ drug_name, section, mode = "patient" }) => {
      ...
      return {
        ...
        section,
        found: !!sectionText,
        pdfUrl: resource.data.pdfUrl || null,
        data: sectionText ? { name: resource.data.name, section, content: sectionText, pdfUrl: resource.data.pdfUrl || null } : { pdfUrl: resource.data.pdfUrl || null },
        message: sectionText ? null : `Seção '${section}' não encontrada na bula de '${drug_name}'.`,
      };
    },
  },
