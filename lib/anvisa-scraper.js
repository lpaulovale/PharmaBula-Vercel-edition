  ...
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      return Array.from(rows).map((row, i) => {
        ...
        const pacienteLink = tds[5]?.querySelector('a[ng-click]');
        const profissionalLink = tds[6]?.querySelector('a[ng-click]');
        return {
          row: i + 1,
          nome: tds[0]?.innerText.trim(),
          empresa: tds[2]?.innerText.trim(),
          situacao: tds[4]?.innerText.trim(),
          hasPacienteLink: !!pacienteLink,
          hasProfissionalLink: !!profissionalLink,
          pacienteNgClick: pacienteLink?.getAttribute('ng-click'),
          profissionalNgClick: profissionalLink?.getAttribute('ng-click'),
        };
      });
    });
    console.log('[ANVISA Scraper] Results:', JSON.stringify(results, null, 2));
    
    // Find first row with patient link (don't assume row 0)
    const targetRow = results.findIndex(r => r.hasPacienteLink && tipo === 'paciente') + 1;
    ...
    // Use the row that actually has the link
    const col = tipo === 'paciente' ? 6 : 7;
    const selector = `table tbody tr[ng-repeat]:nth-child(${targetRow}) td:nth-child(${col}) a[ng-click]`;
    console.log('[ANVISA Scraper] Using row:', targetRow, 'Selector:', selector);
    ...
    try {
      ...
      await browser.close();
      console.error('[ANVISA Scraper] ❌ PDF link not found for row', rowIndex + 1);
      throw new Error(`Bula do ${tipo} não disponível para este medicamento.`);
    }
