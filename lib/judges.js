 ...
 */
async function runJudge(judgeName, context, apiKey, apiUrl, model) {
...
async function runAllJudges(context, apiKey) {
  console.log(`[JUDGE] Running all ${JUDGES.length} judges in parallel...`);

  // Fire all judges in parallel
  const judgePromises = JUDGES.map(judge =>
    runJudge(judge.name, context, apiKey)
